...
> 跨 agent 通用的專案藍圖。任何 agent 的每個 session 都應先讀本檔 ＋ `handoff.md`。

## 這個專案要解決什麼

Claude Code Sessions 管理工具。

<!-- 上面那行是老大給的一句話目標。要補充細節請老大自己填，AI 不要臆測擴寫。 -->
...
- [x] 階段一：掃描索引 + FTS5 全文搜尋 + 本機 Web UI 瀏覽
- [x] 階段二：改名（append `custom-title`）、lzma 歸檔與還原、liveness 保護
- [x] 階段三：排除 Claude Code 自己的 scratchpad 工作區
- [ ] 階段四：待決定事項見 `docs/TODO.md`（圖片區塊、移動專案／刪除原始檔已評估後決定不做）

## 環境與技術棧

Windows 11 + Python venv，FastAPI + 單一自包含 HTML（原生 JS，無建置流程、無 CDN）。

```powershell
Session管理.cmd                                          # 日常入口：雙擊即可（增量索引 + 起服務 + 開瀏覽器）
Session管理.cmd -NoReindex                               # 跳過索引，只開服務
.venv\Scripts\python.exe -m src.cli index                # 手動增量建索引
.venv\Scripts\python.exe -m unittest discover -s tests   # 測試（目前 117 項）
```
...
```powershell
.venv\Scripts\python.exe -m unittest discover -s tests   # 全綠，目前 117 項
.venv\Scripts\python.exe -m src.cli index                # errors 應為 0
```

驗證破壞性功能：**用隔離實例，不要拿真實資料試**。三個路徑都可用環境變數覆寫：
...
- **機密在索引時就遮罩，不是查詢時** —— `index.db` 本身就是產出物。
  代價是遮掉的內容無法從索引還原。
- **索引不會自己跑**。服務啟動（`serve`）只讀不索引，資料會停在最後一次索引的時間點。
  更新途徑：`Session管理.cmd`（已預設增量索引）、UI 的重新索引按鈕、
  `POST /api/reindex`、CLI `index`。**服務運行中要更新索引，走 API 讓服務自己寫，
  不要另起行程同時寫 DB。**
...
- 憑證、API key、明文連線帳密絕不寫入產出物或 commit
- 不得砍 `data/index.db` 重建（紅線 1）；加欄位走 `db.migrate()`
- 不得在未過 `liveness` 檢查的情況下寫入 `~/.claude/` 底下任何檔案（紅線 2）
- 不得連同 `data/labels.db` 一起刪除（紅線 3）
- 不得用真實資料測試破壞性功能（用環境變數起隔離實例）
- 改 `.cmd` 不得引入非 ASCII 位元組或 LF 行尾
...
- 任何 agent：**開工先讀 `handoff.md`，收工必更新 `handoff.md`**
